...
import { deleteSnakeScoreApi, getSnakeScoreListApi } from "@@/apis/game";
...
async function getTableData() {
  ...
  try {
    ...
    const res: any = await getSnakeScoreListApi(params);
...
function handleDelete(row: ScoreData | null, type = "single") {
  ...
  ElMessageBox.confirm(`正在删除成绩记录：${ids}，确认删除？`, "提示", {
    ...
    try {
      loading.value = true;
      const res: any = await deleteSnakeScoreApi({ ids });
      if (res.code === 200) {
